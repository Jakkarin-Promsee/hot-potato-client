...
const EditorFloatingMenu = ({ editor }: EditorFloatingMenuProps) => {
  ...
  useEffect(() => {
    ...
    const plugin = FloatingMenuPlugin({
      ...
      shouldShow: ({ state }) => {
        ...
        const currentLineText = $from.nodeBefore?.textContent || $from.nodeAfter?.textContent;
        const isEmptyLine = !currentLineText && $from.parent.type.name === 'paragraph';
        return isEmptyLine;
      },
      options: {
  ...
  const menuItems = [
    { icon: Image, label: 'Image', action: () => fileInputRef.current?.click() },
